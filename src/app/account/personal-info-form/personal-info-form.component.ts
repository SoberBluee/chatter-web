...
export class PersonalInfoFormComponent implements OnInit {
  ...
  @Output() public emitBannerMessage: EventEmitter<ErrorBannerEvent> =
    new EventEmitter();
  ...
  private initForm(): void {
    this.personalInfoForm = this.fb.group({
      ...
      firstname: [
        this.currentUser?.first_name ? this.currentUser.first_name : "",
      ],
  ...
  public submitPersonalForm(): void {
    ...
    const postData = {
      ...
      phonenumber: this.checkAmberChange("phonenumber")
        ? this.phonenumberVal.toString()
        : null,
    };
  ...
  public checkAmberChange(check: string): boolean {
    switch (check) {
      case "username":
        return (
          this.usernameVal !== this.currentUser?.user_name &&
          this.usernameVal !== ""
        );
      case "firstname":
        return (
          this.firstnameVal !== this.currentUser?.first_name &&
          this.firstnameVal !== ""
        );
      case "surname":
        return (
          this.surnameVal !== this.currentUser?.sur_name &&
          this.surnameVal !== ""
        );
      case "phonenumber":
        return (
          this.phonenumberVal !== this.currentUser?.phonenumber &&
          this.phonenumberVal !== ""
        );
      default:
        return false;
